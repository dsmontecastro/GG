...
func _ready():
	SIGNALS.connect("play", Callable(self, "_play"))
	SIGNALS.connect("reset", Callable(self, "_reset"))
	SIGNALS.connect("exit_opts", Callable(self, "opts_exit"))
	P2P._reset()
	_resize()
	_start()
...
func _test(): SIGNALS.emit_signal("play", "DEBUG")
...
func slideL(object: Control):
	...
	tween.tween_property(object, "position", pos - TWEEN_OFFSET, TWEEN_DURATION)
